...
- [fixed_array.hpp](https://github.com/ismawno/toolkit/blob/main/toolkit/tkit/container/fixed_array.hpp): A fixed size array implementation very similar to `std::array` that plays nicely with the style of the library and implements bound checking that can be easily stripped from distribution or release builds.
