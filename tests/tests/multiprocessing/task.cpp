...
TEST_CASE("Task<T> basic behavior", "[Task]")
{
    ...
    const auto task = Ref<Task<u32>>::Create([](const u32 p_Index) { return u32(p_Index * 2); }, 5u);
...
TEST_CASE("Task<void> basic behavior", "[Task]")
{
    ...
    const auto task = Ref<Task<void>>::Create([&](const u32 p_Index) { counter += p_Index; }, 5u);
    REQUIRE(!task->IsFinished());
...
TEST_CASE("Task::WaitUntilFinished blocks from another thread", "[Task]")
{
    ...
    const auto task = Ref<Task<u32>>::Create([]() {
