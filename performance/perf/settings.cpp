...
{
    TKit::Reflect<Settings>::ForEachField([&p_Settings](const auto &p_Field1) {
        ...
        TKit::Reflect<Type1>::ForEachField([&setting](const auto &p_Field2) {
...
Settings CreateSettings(int argc, char **argv)
{
    ...
    TKit::Reflect<Settings>::ForEachField([&parser, &settings](const auto &p_Field1) {
        ...
        TKit::Reflect<Type1>::ForEachField([&parser, &name, &setting](const auto &p_Field2) {
            using Type2 = TKIT_REFLECT_FIELD_TYPE(p_Field2);
            const std::string value = std::to_string(p_Field2.Get(setting));
    ...
    TKit::Reflect<Settings>::ForEachField([&parser, &settings](const auto &p_Field1) {
        ...
        const std::string name = p_Field1.Name;
        TKit::Reflect<Type1>::ForEachField([&parser, &setting, &name](const auto &p_Field2) {
            using Type2 = TKIT_REFLECT_FIELD_TYPE(p_Field2);
