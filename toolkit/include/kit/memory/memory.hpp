...
KIT_API void *Allocate(size_t p_Size) KIT_NOEXCEPT;
KIT_API void Deallocate(void *p_Ptr) KIT_NOEXCEPT;

KIT_API void *AllocateAligned(size_t p_Size, size_t p_Alignment) KIT_NOEXCEPT;
...
template <typename T> constexpr size_t AlignedSize() KIT_NOEXCEPT
{
    constexpr size_t remainder = sizeof(T) % alignof(T);
