...
#ifndef TKIT_HASH_MAP_LOAD_FACTOR_THRESHOLD
#    define TKIT_HASH_MAP_LOAD_FACTOR_THRESHOLD 0.7f
#endif
...
template <typename K, typename V> struct MapNode
{
    ...
    MapNodeState State = MapNode_Free;
  ...
  public:
    ...
    template <typename T> class IteratorImpl
    {
      public:
        ...
        {
            if (m_Buckets->At(m_Index).State != MapNode_Occupied)
                ++(*this);
        }
        ...
        IteratorImpl &operator++()
        {
            for (u32 i = m_Index + 1; i < m_Buckets->GetSize(); ++i)
                if (m_Buckets->At(i).State == MapNode_Occupied)
                {
                    m_Index = i;
                    return *this;
                }
    ...
    constexpr HashMap(const usize capacity) : m_Buckets(capacity)
    {
        TKIT_ASSERT(IsPowerOfTwo(capacity),
                    "[TOOLKIT][HASH-MAP] The size of the underlying array must be a power of 2");
        setEmpty();
    }
    constexpr HashMap(AllocState &&state) : m_Buckets(std::move(state))
    {
    }
    constexpr HashMap(const usize capacity, AllocState &&state) : m_Buckets(capacity, std::move(state))
    {
        TKIT_ASSERT(IsPowerOfTwo(capacity),
                    "[TOOLKIT][HASH-MAP] The size of the underlying array must be a power of 2");
        setEmpty();
    ...
    constexpr V *TryInsert(const K &key, Args &&...args)
    {
        ...
        const usize idx = usize(hash & (size - 1));

        Node &node = m_Buckets->At(idx);
        if (node.State == MapNode_Occupied)
            return nullptr;

        ++m_Size;
        node.Hash = hash;
        node.State = MapNode_Occupied;
  ...
  private:
    ...
    usize find(const K &key, const usz hash) const
    {
        ...
        const auto tryFind = [&](const usize i) {
            ...
            if (node.State == MapNode_Free)
                return true;
            if (node.State == MapNode_Tombstone || node.Hash != hash || key != node.GetEntry()->Key)
                return false;
    ...
    {
        const usize size = maybeRehash();
        const usize idx = usize(hash & (size - 1));
        ++m_Size;
        TKIT_ASSERT(m_Size <= size,
                    "[TOOLKIT][HASH-MAP] The size of the hash map ({}) exceeds the size of the underlying array ({})",
                    m_Size, size);
        ...
        requires(Type != Array_Arena && Type != Array_Stack)
    {
        ...
        const usize size = m_Buckets.GetSize();
        const usize nsize = size == 0 ? 16 : (2 * size);

        TKIT_ASSERT(IsPowerOfTwo(nsize), "[TOOLKIT][HASH-MAP] The size of the underlying array must be a power of 2");
    ...
    constexpr usize maybeRehash()
    {
        ...
        {
            if (!m_Buckets.IsEmpty() && GetLoadFactor() < TKIT_HASH_MAP_LOAD_FACTOR_THRESHOLD)
                return m_Buckets.GetSize();
            return rehash();
        }
    }
